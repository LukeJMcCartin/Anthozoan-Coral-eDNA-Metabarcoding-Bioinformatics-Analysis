...
This R markdown file accompanies McCartin et al. 2023 "Nuclear eDNA Metabarcoding Primers for Anthozoan Coral Biodiversity Assessment". It assumes that you have generated amplicon sequencing data generated on an Illumina platform and that the data are demultiplexed. It is suitable for the analysis of anthozoan coral metabarcoding data generated using the Anth-28S-eDNA or Scler-28S-eDNA primers described in our paper, or using the mtMutS primers described in Everett and Park (2018), published in Deep-sea Research Part II. It requires a number of commonly used bioinformatic packages that are implemented in either bash or R notebook chunks.

For the sake of this demonstration, I am going to analyze eDNA metabarcoding libraries generated using the Anth-28S-eDNA primers. For the other primer sets, simply replace the primer sequences at the primer-trimming step and replace the reference database files for taxonomic classification.
...
q_plot_F <- plotQualityProfile(fnFs[c(1:4)]) #plotting quality of forward reads

pdf(paste("./q_plot_F.pdf"), width = 11, height = 8.5)
print(q_plot_F)
dev.off()

q_plot_R <- plotQualityProfile(fnRs[c(1:4)]) #plotting quality of reverse reads

pdf(paste("./q_plot_R.pdf"), width = 11, height = 8.5)
print(q_plot_R)
dev.off()
...
e_plot_F <- plotErrors(errF) #plotting the fit of the forward error model

pdf(paste("./e_plot_F.pdf"), width = 11, height = 8.5)
print(e_plot_F)
dev.off()

e_plot_R <- plotErrors(errR) #plotting the fit of the reverse error model

pdf(paste("./e_plot_R.pdf"), width = 11, height = 8.5)
print(e_plot_R)
dev.off()
